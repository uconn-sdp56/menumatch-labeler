...
data "archive_file" "get_dataset_item" {
  type        = "zip"
  source_dir  = "${path.module}/../aws/lambdas/get_dataset_item"
...
resource "aws_lambda_function" "get_dataset_item" {
  function_name = "${local.name_prefix}-get_dataset_item"
  role          = aws_iam_role.lambda_exec.arn
  runtime       = "python3.11"
  handler       = "get_dataset_item.lambda_handler"

  filename         = data.archive_file.get_dataset_item.output_path
  source_code_hash = data.archive_file.get_dataset_item.output_base64sha256
...
resource "aws_apigatewayv2_integration" "get_dataset_item" {
  api_id                 = aws_apigatewayv2_api.this.id
  integration_type       = "AWS_PROXY"
  integration_uri        = aws_lambda_function.get_dataset_item.invoke_arn
  integration_method     = "POST"
  payload_format_version = "2.0"
}
...
resource "aws_apigatewayv2_route" "get_dataset_item" {
  api_id    = aws_apigatewayv2_api.this.id
...
resource "aws_lambda_permission" "get_dataset_item" {
